...
class Check extends MY_Controller{
	...
	public function index(){
		// $params = array(
			// 'id_zone' => '5a12b22945fbbe3f6ad5dab9',
			// 'client_id' => '5a12b20b45fbbef553d5dab7',
			// 'resller_id' => '59995d7745fbbed1233e1db3',
		// );
		// $param = array(
	...
	public function test(){
		$file = fopen("/var/www/clients/client0/web1/web/mingrate_cpanel/check_zone.txt", "r");
		while (!feof($file)) {
			$zone = fgets($file);
			$domain = $this->punny_code(trim(strtolower($zone)));
			$filenew = '/var/named/uzone/pri.'.$domain;
			if(file_exists($filenew)){
				log_convert('check_zone-yes', $domain);
			}else{
				log_convert('check_zone-none', $domain);
			}
	...
	public function convert_zone_sign(){
		// php /var/www/clients/client0/web1/web/apps/index.php apps check convert_zone_sign
		$name_zone = "xn--chnlngcu-73a1m4226a.vn";
		// $name_zone = $this->convert_string($this->punny_code($zone));
		$response_zone = $this->mongo_db->where(array('name_zone' => $name_zone))->get('ureg_zone');
		if(!empty($response_zone)){
			...
			echo "zone_active";
			log_convert('write_zone_yes', $name_zone);
			}else{
				echo "zone_error";
				log_convert('write_zone_no-',$name_zone);	
			}
		}else{
			echo "zone_not_found";
			log_convert('write_zone_no', $name_zone);	
